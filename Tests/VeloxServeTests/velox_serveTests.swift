...
final class VeloxServeTests {
    ...
    private func withServer<T>(
        handler: @escaping Handler, client: @escaping SimpleClient.ClientHandler<T>
    ) async throws -> T {
            ...
            try await outbound.write(
                .head(
                    HTTPRequest(
                        ...
                        path: "/?name=value&test=M%C3%BCnchen&name=second+value")))
    ...
    func testEmptyQueryParsing() async throws {
        let result = try await withServer { req, res in
            ...
            try await outbound.write(
                .head(HTTPRequest(method: .get, scheme: nil, authority: nil, path: "/?")))
            try await outbound.write(.end(nil))
    ...
    func testInterceptingHandler() async throws {
        ...
        @Sendable
        func reqBodyLogger(
            _ req: any RequestReader, _ res: any ResponseWriter, _ next: Handler
        ) async throws {
            ...
            for _ in 0..<N {
                group.addTask { [client] in
                    let req = HTTPClientRequest(
                        url: "http://localhost:\(server.localAddress!.port!)/")
                    let response = try await client!.execute(req, deadline: .now() + .seconds(2))
                    let body = try await response.body.collect(upTo: 1024).readableBytesView
    ...
    func testTrieRouter() async throws {
        ...
        router.get(
            ...
            handler: AnyHandler { req, res in
                ...
                try await res.plainText(
                    "User \(req.pathParameters["id"]!) post \(req.pathParameters["postId"]!)")
            })
        ...
        router.get(
            ...
            handler: AnyHandler { req, res in
                ...
                try await res.plainText(
                    "User \(req.pathParameters["id"]!) post \(req.pathParameters["postId"]!) comments"
                )
            })
        ...
        try await withServer(handler: router.handle) { inbound, outbound in
            ...
            #expect(response.0.status == .ok)
            #expect(
                "User 123 posts" == String(decoding: response.1.readableBytesView, as: UTF8.self))
            ...
            #expect(response.0.status == .ok)
            #expect(
                "User 123 post abc" == String(decoding: response.1.readableBytesView, as: UTF8.self)
            )
            ...
            #expect(response.0.status == .ok)
            #expect(
                "User 123 post abc comments"
                    == String(decoding: response.1.readableBytesView, as: UTF8.self))
    ...
    func testMethodNotAllowed() async throws {
        ...
        router.get(
            ...
            handler: AnyHandler { req, res in
                try await res.plainText(
                    "OK: /users/\(req.pathParameters[required: "id", as: Int.self])/posts")
            })
...
enum SimpleClient {
    typealias ClientHandler<Result> = (
        NIOAsyncChannelInboundStream<HTTPResponsePart>,
        NIOAsyncChannelOutboundWriter<HTTPRequestPart>
    ) async throws -> (Result)

    static func execute<Result>(host: String, port: Int, _ work: @escaping ClientHandler<Result>)
        async throws -> Result
    {
        let channel: NIOAsyncChannel = try await ClientBootstrap(
...
extension NIOAsyncChannelOutboundWriter<HTTPRequestPart> {
    ...
    func post(_ path: String, body: ByteBuffer) async throws {
        try await self.write(
            .head(HTTPRequest(method: .post, scheme: nil, authority: nil, path: path)))
    ...
    func head(_ path: String) async throws {
        try await self.write(
            .head(HTTPRequest(method: .head, scheme: nil, authority: nil, path: path)))
        try await self.write(.end(nil))
    }
}
